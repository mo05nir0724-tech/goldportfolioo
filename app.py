...
def speichere_daten(datei, items, ist_verkauf=False):
  try:
    with open(datei, "w", encoding="utf-8", newline="") as f:
      ...
      for item in items:
        if ist_verkauf:
          writer.writerow([item.name, item.typ, item.gewicht_gramm, item.kaufdatum, item.kaufpreis, item.verkaufspreis, item.verkauf_datum])
        else:
          writer.writerow([item.name, item.typ, item.gewicht_gramm, item.datum, item.kaufpreis, item.manueller_wert])
  except Exception:
    pass
      ...
      with st.expander("Eintrag verkaufen oder löschen", expanded=False):
        ...
        if st.button("💵 Als verkauft buchen", type="primary", use_container_width=True):
          ...
          verkauftes_item = VerkaufsItem(
              name=item_zu_verkaufen.name, typ=item_zu_verkaufen.typ, gewicht_gramm=item_zu_verkaufen.gewicht_gramm,
